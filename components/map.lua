...
function Map:RegisterDeployExtraSpacing(spacing) DEPLOY_EXTRA_SPACING = math.max(spacing, DEPLOY_EXTRA_SPACING) end
...
function Map:RegisterTerraformExtraSpacing(spacing) TERRAFORM_EXTRA_SPACING = math.max(spacing, TERRAFORM_EXTRA_SPACING) end
...
function Map:IsPassableAtPointWithPlatformRadiusBias(x, y, z, allow_water, exclude_boats, platform_radius_bias,
    ...
    if not allow_water and not valid_tile then
        if not exclude_boats then
            local entities = TheSim:FindEntities(x, 0, z, TUNING.MAX_WALKABLE_PLATFORM_RADIUS + platform_radius_bias,
...
function Map:CanTerraformAtPoint(x, y, z)
    ...
    elseif TERRAFORM_EXTRA_SPACING > 0 then
        for i, v in ipairs(TheSim:FindEntities(x, 0, z, TERRAFORM_EXTRA_SPACING, TERRAFORMBLOCKER_TAGS,
            TERRAFORMBLOCKER_IGNORE_TAGS)) do
            if v.entity:IsVisible() and v:GetDistanceSqToPoint(x, 0, z) < v.terraform_extra_spacing *
                v.terraform_extra_spacing then
                return false
            end
        end
    end
...
function Map:IsTerraformingBlockedByAnObject(tile_x, tile_y)
    if TERRAFORM_EXTRA_SPACING <= 0 then
        return false
    end

    local cx, _, cz = self:GetTileCenterPoint(tile_x, tile_y)
    for _, blocker in ipairs(TheSim:FindEntities(cx, 0, cz, TERRAFORM_EXTRA_SPACING, TERRAFORMBLOCKER_TAGS,
        TERRAFORMBLOCKER_IGNORE_TAGS)) do
        if blocker.entity:IsVisible() and blocker:GetDistanceSqToPoint(cx, 0, cz) < blocker.terraform_extra_spacing *
            blocker.terraform_extra_spacing then
            return true
        end
    end
...
function Map:CanPlowAtPoint(x, y, z)
    if not self:CanPlantAtPoint(x, y, z) then
        return false
    elseif TERRAFORM_EXTRA_SPACING > 0 then
        for _, v in ipairs(TheSim:FindEntities(x, 0, z, TERRAFORM_EXTRA_SPACING, TERRAFORMBLOCKER_TAGS,
            TERRAFORMBLOCKER_IGNORE_TAGS)) do
            if v.entity:IsVisible() and v:GetDistanceSqToPoint(x, 0, z) < v.terraform_extra_spacing *
                v.terraform_extra_spacing then
                return false
...
function Map:CanPlaceTurfAtPoint(x, y, z) return self:GetTileAtPoint(x, y, z) == WORLD_TILES.DIRT end
...
function Map:IsFarmableSoilAtPoint(x, y, z) return self:GetTileAtPoint(x, y, z) == WORLD_TILES.FARMING_SOIL end
...
function Map:CanTillSoilAtPoint(x, y, z, ignore_tile_type)
    return (ignore_tile_type and self:CanPlantAtPoint(x, y, z) or self:IsFarmableSoilAtPoint(x, y, z)) and
               self:IsDeployPointClear(Vector3(x, y, z), nil, GetFarmTillSpacing(), nil, nil, nil, TILLSOIL_IGNORE_TAGS)
end
...
function Map:IsGroundTargetBlocked(pt, range)
    range = range or .5
    for _, blocker in ipairs(TheSim:FindEntities(pt.x, 0, pt.z, math.max(DEPLOY_EXTRA_SPACING,
        MAX_GROUND_TARGET_BLOCKER_RADIUS) + range, nil, nil, BLOCKED_ONEOF_TAGS)) do
        local radius = (blocker.ground_target_blocker_radius or blocker._groundhole_outerradius or
                           blocker:GetPhysicsRadius(0)) + (blocker._groundhole_rangeoverride or range)
...
function Map:IsDeployPointClear(pt, inst, min_spacing, min_spacing_sq_fn, near_other_fn, check_player,
    ...
    for _, v in ipairs(TheSim:FindEntities(pt.x, 0, pt.z, math.max(DEPLOY_EXTRA_SPACING, min_spacing), nil,
        (custom_ignore_tags ~= nil and custom_ignore_tags) or (check_player and DEPLOY_IGNORE_TAGS_NOPLAYER) or
            DEPLOY_IGNORE_TAGS)) do
...
local function IsNearOther2(other, pt, object_size)
    -- FindEntities range check is <=, but we want <
    object_size = object_size + (other.deploy_smart_radius or other.deploy_extra_spacing or
                      (other.replica.inventoryitem and other:GetPhysicsRadius(0.5)) or 0)
...
function Map:IsDeployPointClear2(pt, inst, object_size, object_size_fn, near_other_fn, check_player, custom_ignore_tags)
    ...
    for i, v in ipairs(TheSim:FindEntities(pt.x, 0, pt.z, entities_radius, nil,
        (custom_ignore_tags ~= nil and custom_ignore_tags) or (check_player and DEPLOY_IGNORE_TAGS_NOPLAYER) or
            DEPLOY_IGNORE_TAGS)) do
        if v ~= inst and v.entity:IsVisible() and v.components.placer == nil and v.entity:GetParent() == nil and
...
function Map:CanDeployAtPoint(pt, inst, mouseover)
    local x, y, z = pt:Get()
    return (mouseover == nil or mouseover:HasTag("player") or mouseover:HasTag("walkableplatform") or
               mouseover:HasTag("walkableperipheral")) and
               self:IsPassableAtPointWithPlatformRadiusBias(x, y, z, false, false, TUNING.BOAT.NO_BUILD_BORDER_RADIUS,
            true) and self:IsDeployPointClear(pt, inst,
...
function Map:CanDeployPlantAtPoint(pt, inst)
    return self:CanPlantAtPoint(pt:Get()) and self:IsDeployPointClear(pt, inst,
        inst.replica.inventoryitem ~= nil and inst.replica.inventoryitem:DeploySpacingRadius() or
            DEPLOYSPACING_RADIUS[DEPLOYSPACING.DEFAULT])
end
...
function Map:CanDeployWallAtPoint(pt, inst)
    ...
    local x, y, z = pt:Get()
    local ispassable, is_overhang = self:IsPassableAtPointWithPlatformRadiusBias(x, y, z, false, false,
        TUNING.BOAT.NO_BUILD_BORDER_RADIUS, false)
...
function Map:CanDeployAtPointInWater(pt, inst, mouseover, data)
    ...
    local entities = TheSim:FindEntities(pt.x, 0, pt.z,
        TUNING.MAX_WALKABLE_PLATFORM_RADIUS + radius + min_distance_from_boat, WALKABLE_PLATFORM_TAGS)
    for i, v in ipairs(entities) do
        if v.components.walkableplatform and math.sqrt(v:GetDistanceSqToPoint(pt.x, 0, pt.z)) <=
            (v.components.walkableplatform.platform_radius + radius + min_distance_from_boat) then
            return false
        end
    end
...
function Map:CanDeployMastAtPoint(pt, inst, mouseover)
    ...
    return (mouseover == nil or mouseover:HasTag("player") or mouseover:HasTag("walkableplatform") or
               mouseover:HasTag("walkableperipheral")) and
               self:IsPassableAtPointWithPlatformRadiusBias(pt.x, pt.y, pt.z, false, false,
            TUNING.BOAT.NO_BUILD_BORDER_RADIUS, true) and
               self:IsDeployPointClear(pt, nil, inst.replica.inventoryitem:DeploySpacingRadius())
end
...
function Map:CanDeployWalkablePeripheralAtPoint(pt, inst)
    return self:IsDeployPointClear(pt, nil, inst.replica.inventoryitem:DeploySpacingRadius(), nil,
        IsNearOtherWalkablePeripheral, nil, WALKABLEPERIPHERAL_DEPLOY_IGNORE_TAGS)
end

local function IsDockNearOtherOnOcean(other, pt, min_spacing_sq, min_spacing)
    return IsNearOther(other, pt, min_spacing_sq, min_spacing) and
...
function Map:CanDeployDockAtPoint(pt, inst, mouseover)
    ...
    for _, v in ipairs(boat_entities) do
        if v.components.walkableplatform ~= nil and math.sqrt(v:GetDistanceSqToPoint(pt.x, 0, pt.z)) <=
            (v.components.walkableplatform.platform_radius + min_distance_from_entities) then
            return false
        end
    end
    ...
    return (mouseover == nil or mouseover:HasTag("player")) and
               self:IsDeployPointClear(pt, nil, min_distance_from_entities, nil, IsDockNearOtherOnOcean)
end
...
function Map:CanDeployBridgeAtPointWithFilter(pt, inst, mouseover, tilefilterfn)
    ...
    return (mouseover == nil or mouseover:HasTag("player")) and
               self:IsDeployPointClear(pt, nil, min_distance_from_entities)
end
...
function Map:BridgeFilter_Void(tile) return TileGroupManager:IsInvalidTile(tile) end
...
function Map:CanDeployBoatAtPointInWater(pt, inst, mouseover, data)
    ...
    return (mouseover == nil or mouseover:HasTag("player")) and
               self:IsDeployPointClear2(pt, nil, boat_radius + boat_extra_spacing, nil, nil, nil, BOAT_IGNORE_TAGS) and
               self:IsSurroundedByWater(pt.x, pt.y, pt.z, boat_radius + min_distance_from_land)
end
...
function Map:CanDeployRecipeAtPoint(pt, recipe, rot)
    ...
    if BUILDMODE.WATER == recipe.build_mode then
        ...
        local pt_x, pt_y, pt_z = pt:Get()
        is_valid_ground = self:IsPassableAtPointWithPlatformRadiusBias(pt_x, pt_y, pt_z, false, false,
...
function Map:IsSurroundedByWater(x, y, z, radius)
    ...
    if not self:IsOceanTileAtPoint(x + radius, y, z + radius) then
        return false
    end
    if not self:IsOceanTileAtPoint(x - radius, y, z + radius) then
        return false
    ...
    for i = 1, num_edge_points do
        local idist = dist * i
        if not self:IsOceanTileAtPoint(x - radius + idist, y, z + radius) then
            return false
        end
        if not self:IsOceanTileAtPoint(x - radius + idist, y, z - radius) then
            return false
        end
        if not self:IsOceanTileAtPoint(x - radius, y, z - radius + idist) then
            return false
        end
        if not self:IsOceanTileAtPoint(x + radius, y, z - radius + idist) then
            return false
    ...
    for i = 1, num_edge_points do
        ...
        for j = 1, num_edge_points do
            local jdist = dist * j
            if not self:IsOceanTileAtPoint(x - radius + idist, y, z - radius + jdist) then
                return false
            end
        end
    end
...
function Map:IsSurroundedByLand(x, y, z, radius)
    ...
    -- test the corners first
    if not self:IsLandTileAtPoint(x + radius, y, z + radius) then
        return false
    end
    if not self:IsLandTileAtPoint(x - radius, y, z + radius) then
        return false
    ...
    for i = 1, num_edge_points do
        local idist = dist * i
        if not self:IsLandTileAtPoint(x - radius + idist, y, z + radius) then
            return false
        end
        if not self:IsLandTileAtPoint(x - radius + idist, y, z - radius) then
            return false
        end
        if not self:IsLandTileAtPoint(x - radius, y, z - radius + idist) then
            return false
        end
        if not self:IsLandTileAtPoint(x + radius, y, z - radius + idist) then
            return false
    ...
    for i = 1, num_edge_points do
        ...
        for j = 1, num_edge_points do
            local jdist = dist * j
            if not self:IsLandTileAtPoint(x - radius + idist, y, z - radius + jdist) then
                return false
            end
        end
    end
...
function Map:GetPlatformAtPoint(pos_x, pos_y, pos_z, extra_radius)
    ...
    local entities = TheSim:FindEntities_Registered(pos_x, pos_y, pos_z,
        TUNING.MAX_WALKABLE_PLATFORM_RADIUS + (extra_radius or 0), REGISTERED_WALKABLE_PLATFORM_TAGS)
    for i, v in ipairs(entities) do
...
function Map:GetNearestPlatformInDirection(x, z, forward_x, forward_z, dist)
    for i, v in ipairs(TheSim:FindEntities_Registered(x, 0, z, dist + TUNING.MAX_WALKABLE_PLATFORM_RADIUS,
        REGISTERED_WALKABLE_PLATFORM_TAGS)) do
...
function Map:FindRandomPointInOcean(max_tries) return self:FindRandomPointWithFilter(max_tries, self.IsOceanAtPoint) end

function Map:FindRandomPointOnLand(max_tries) return self:FindRandomPointWithFilter(max_tries, self.IsLandTileAtPoint) end
...
function Map:CanAreaTagsHaveQuaker(tags) return not table.contains(tags, "noquaker") end
...
function Map:FindVisualNodeAtPoint(x, y, z, has_tag)
    ...
    local best = FindVisualNodeAtPoint_TestArea(self, x, z, on_land, 0.95) or
...
function Map:IsInLunacyArea(x, y, z)
    return
        (TheWorld.state.isalterawake and TheWorld.state.isnight) or self:FindVisualNodeAtPoint(x, y, z, "lunacyarea") ~=
            nil
end
...
function Map:IsTileLandNoDocks(tile) return TileGroupManager:IsLandTile(tile) and tile ~= WORLD_TILES.MONKEY_DOCK end

function Map:IsTileOcean(tile) return TileGroupManager:IsOceanTile(tile) end
...
function Map:GetGoodArenaPoints() return GoodArenaPoints, GoodArenaPoints_Count end
...
function Map:CheckForBadThingsInArena(pt, badthingsatspawnpoints)
    ...
    if self:IsAboveGroundInSquare(x, y, z, GOOD_ARENA_SQUARE_SIZE, self.IsTileLandNoDocks) and
        not IsAnyPlayerInRange(x, y, z, NO_PLAYER_RADIUS) then
        local badthings = TheSim:FindEntities(x, y, z, IS_CLEAR_AREA_RADIUS, nil, BADARENA_CANT_TAGS,
            BADARENA_ONEOF_TAGS)
...
function Map:GetGoodOceanArenaPoints() return GoodOceanArenaPoints, GoodOceanArenaPoints_Count end
...
function Map:CheckForBadThingsInOceanArena(pt, badthingsatspawnpoints)
    local x, y, z = pt.x, pt.y, pt.z
    if self:IsAboveGroundInSquare(x, y, z, GOOD_ARENA_SQUARE_SIZE, self.IsTileOcean) and
        not IsAnyPlayerInRange(x, y, z, NO_PLAYER_RADIUS) then
...
local function IsVaultTile(tileid) return tileid == WORLD_TILES.VAULT end
